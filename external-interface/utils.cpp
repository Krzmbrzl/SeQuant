...
std::string to_string(const IndexSpace &space) {
	...
	stream << "b" << std::bitset< 32 >(space.type().to_int32());
	return stream.str();
}
...
std::size_t IndexSpaceMeta::getSize(const IndexSpace &space) const {
	...
	if (iter == m_entries.end()) {
		throw std::runtime_error("No known size for index space " + to_string(space));
	}
...
std::wstring IndexSpaceMeta::getLabel(const IndexSpace &space) const {
	auto iter = m_entries.find(space);
	if (iter == m_entries.end()) {
		throw std::runtime_error("No known label for index space " + to_string(space));
	}
...
std::wstring IndexSpaceMeta::getName(const IndexSpace &space) const {
	auto iter = m_entries.find(space);
	if (iter == m_entries.end()) {
		throw std::runtime_error("No known name for index space " + to_string(space));
	}
...
std::wstring IndexSpaceMeta::getTag(const IndexSpace &space) const {
	auto iter = m_entries.find(space);
	if (iter == m_entries.end()) {
		throw std::runtime_error("No known tag for index space " + to_string(space));
	}
...
sequant::ExprPtr generateResultSymmetrization(std::wstring_view precursorName,
	...
	for (std::size_t i = 0; i < externals.bra.size(); ++i) {
		...
		symmetrization += ex< Tensor >(precursorName, std::move(symKet), std::move(symBra), externals.aux);
	}
