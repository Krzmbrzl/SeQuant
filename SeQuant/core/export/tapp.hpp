...
class TappGenerator : public Generator<Context> {
 public:
  ...
  void declare(const Tensor &tensor, UsageSet usage,
	...
	for (std::size_t i = 0; i < tensor.indices().size(); ++i) {
		const Index &idx = tensor.indices().at(i);
