...
Tensor generateResultTensor(ExprPtr expr) {
  ...
  return Tensor(L"Result", std::move(externals.bra), std::move(externals.ket), std::move(externals.aux));
}
...
std::vector<Contraction> to_contractions(const Product &product,
  ...
  for (const ExprPtr &factor : product.factors()) {
    if (factor.is<Product>()) {
      ...
      // indices in the aux group for now
      Tensor intermediate(intermediateName.data(), std::vector<Index>{},
                          std::vector<Index>{}, std::move(intermediateIndices));
...
void one_electron_integral_remapper(
  ...
  expr =
      ex<Tensor>(tensor.label(), std::move(braIndices), std::move(ketIndices), std::vector<Index>{});
}
...
void two_electron_integral_remapper(
  ...
  expr = ex<Tensor>(std::move(tensorLabel), std::move(braIndices),
                    std::move(ketIndices), std::vector<Index>{});
}
