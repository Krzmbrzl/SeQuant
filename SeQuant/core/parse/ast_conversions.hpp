...
ResultExpr ast_to_result(const parse::ast::ResultExpr &result,
                     const PositionCache &position_cache, const Iterator &begin,
                     Symmetry default_symmetry) {
	ExprPtr lhs = std::visit(Transformer<PositionCache, Iterator>{
              std::ref(position_cache), std::ref(begin),
              std::ref(default_symmetry)}, result.lhs);
	ExprPtr rhs = ast_to_expr(result.rhs, position_cache, begin, default_symmetry);
